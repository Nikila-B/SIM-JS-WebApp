class Box
{
    ...
    add_people(num,inf)
    {
        this.p_x = tf.concat([this.p_x,tf.randomUniform([num], this.pos[0], this.pos[0]+this.size[0],'float32')]);
        this.p_y = tf.concat([this.p_y,tf.randomUniform([num], this.pos[1],this.pos[1]+this.size[1],'float32')]);
        this.p_state = tf.concat([tf.tensor(this.p_state),tf.multinomial(tf.tensor([.9,.1]),num)]).arraySync();
        this.num+=num;
    }
    get_info()
    {
      
        return tf.stack([this.p_x,this.p_y,tf.tensor(this.p_state)]).transpose().arraySync();
    }
    get_box_info(){
    ...
    {
        var sum_x = tf.add(this.p_x,displacement[0]);
        var sum_y = tf.add(this.p_y,displacement[1]);
        this.p_x = this.bounce_x((sum_x).arraySync());
        this.p_y = this.bounce_x((sum_y).arraySync());                     
    ...
    update_state(r,t,p_r,p_t)
    {
        ...
        for(x in inf_cur)
        {
            this.p_state[inf_cur[x]]=tf.multinomial(tf.tensor([1-p_r,p_r]),1).arraySync()[0] //probability
            this.p_inf_time[inf_cur[x]]=this.now
        }
        console.log(this.p_state)
    }
    ...
    set_dest(max)
    {
        ...
        {
            x = tf.randomNormal([1],this.p_x.arraySync()[i],tf.randomUniform([1],0,max).arraySync()[0]).arraySync()[0];
            y = tf.randomNormal([1],this.p_y.arraySync()[i],tf.randomUniform([1],0,max).arraySync()[0]).arraySync()[0];
            dest_x.push(x);
            dest_y.push(y);
        }
        this.p_dest_x = tf.tensor(dest_x);
        this.p_dest_y = tf.tensor(dest_y);
        this.set_dest_bounds()
    }

    set_dest_bounds()
    {
        this.p_dest_x = this.bounce_x(this.p_dest_x.arraySync())
        this.p_dest_y = this.bounce_y(this.p_dest_y.arraySync())
    }
    ...
    move_to_dest(speed)
    {
        ...
        var disp_y
        s = tf.scalar(speed);
        disp_x = tf.mul(s,tf.sub(this.p_dest_x,this.p_x));
        disp_y = tf.mul(s,tf.sub(this.p_dest_y,this.p_y));
